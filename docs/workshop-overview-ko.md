...
1. FastAPI 기본 애플리케이션과 업로드 API를 만든다.
2. `curl`로 영상을 업로드하고 저장 결과를 확인한다.
3. Redis와 Celery worker를 연결한다.
4. API가 작업을 enqueue하고 즉시 `202 Accepted`를 반환하도록 변경한다.
5. worker에서 FFmpeg를 실행한다.
6. 작업별 디렉터리에 HLS manifest와 segment를 생성한다.
7. 상태 API로 `PENDING`, `STARTED`, `SUCCESS`, `FAILURE`를 확인한다.
8. 웹 페이지에서 상태를 polling한다.
9. 원본 영상과 HLS 결과를 나란히 재생한다.
...
- `checkpoint/04-hls-player`: polling, hls.js, 전후 비교 UI
